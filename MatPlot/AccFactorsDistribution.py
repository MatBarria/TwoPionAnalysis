...
def AccFactorsDistribution():
    ...
    for factor in vars:
        ...
        for j in range(nPion):  # Loops on the number of pions
            for i in range(3):  # Loops on the diffrent targets
                with ur.open(inputDirectory +
                    ...
                    totalFactors = np.concatenate(
                        (totalFactors, factors[factor]), axis=0)
        ...
        fig.savefig(outputDirectory + factor +
                    "distribution_.pdf", bbox_inches='tight')
...
def FinalFactorsDistribution():
    ...
    for j in range(nPion):  # Loops on the number of pions
        for i in range(3):  # Loops on the diffrent targets
            with ur.open(inputDirectory +
                         "AccTuple.root:ntuple_" + tarList[i] + "_" + str(j+1)) as file:
                factors = file.arrays(vars, library="np")
                FFactor = np.concatenate(
                    (FFactor, factors['FinalFactor']), axis=0)
                Zh = np.concatenate(
                    (Zh, factors['Zh']), axis=0)
        print("-----" + str(j) + "----" + str(i))
        print(len(FFactor))
        print(len(Zh))
        Zh = Zh[FFactor != 0]
    ...
    fig.savefig(outputDirectory + "FinalFactordistribution_.pdf",
                bbox_inches='tight')
