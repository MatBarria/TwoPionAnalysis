...
def PtBroad2DZhSum():
    ...
    for i in range(3): # Loops on the diffrent targets
        ...
        hBroad = axs.hist2d(data['Zh'], data['Zh_1'], bins = (binsX, binsY), weights = data['Broad'] , cmap=cm) 
        ...
        for j in range(len(hBroad[2])-1):
            for k in range(len(hBroad[1])-1):
                ...
                if j<= k+1 and hBroad[1][k+1]/2. <= hBroad[2][j+1]: 
                    ...
                    if flag1:
                        axs.fill_between((hBroad[1][k],hBroad[1][k+1]), 1, hBroad[2][j],color='grey', label = '$Zh_1>Zh_{Sum}$')
                        flag1 = False
                    else:
                        axs.fill_between((hBroad[1][k],hBroad[1][k+1]), 1, hBroad[2][j], color = 'grey') 
                else:
                    if flag2:
                        axs.fill_between((hBroad[1][k],hBroad[1][k+1]),hBroad[2][j+1], 0, color = 'lightgrey', label = '$Zh_2>Zh_1$')
                        flag2 = False
                    else:
                        axs.fill_between((hBroad[1][k],hBroad[1][k+1]),hBroad[2][j+1], 0, color = 'lightgrey')
                # elif k < jbb:
                    # print("hola")
    ...
    # Set the labels for the three plots
        ...
        axs.tick_params(right = False, top = False, left = False, bottom = False,which = 'minor')
        ...
        fig.savefig(outputDirectory + "PtBroad2DZhSum_" + tarList[i]+ "-w.pdf", bbox_inches = 'tight')
        print(outputDirectory + "PtBroad2DZhSum-w.pdf Has been created")

def PtBroad2DZhSumOne():

    fig, axs = plt.subplots(1, 3, sharey = 'row', sharex = 'col', gridspec_kw={'width_ratios': [4, 4, 5]})
    ...
    fig.subplots_adjust(left = None, bottom = None, right = None, top = None, wspace = 0.04, hspace = 0.02)
    ...
    for i in range(3): # Loops on the diffrent targets
        ...
        # Generate the plot
        hError = plt.hist2d(data['Zh'], data['Zh_1'], bins = (binsX, binsY), weights = data['Error']) 
        hBroad = axs[i].hist2d(data['Zh'], data['Zh_1'], bins = (binsX, binsY), weights = data['Broad'] , cmap=cm, vmax =0.0884 ) 
        ...
        for j in range(len(hBroad[2])-1):
            for k in range(len(hBroad[1])-1): 
                ...
                if j<= k+1 and hBroad[1][k+1]/2. <= hBroad[2][j+1]: 
                    ...
                    if flag1:
                        axs[i].fill_between((hBroad[1][k],hBroad[1][k+1]), 1, hBroad[2][j],color='grey', label = '$Zh_1>Zh_{Sum}$')
                        flag1 = False
                    else:
                        axs[i].fill_between((hBroad[1][k],hBroad[1][k+1]), 1, hBroad[2][j], color = 'grey') 
                else:
                    if flag2:
                        axs[i].fill_between((hBroad[1][k],hBroad[1][k+1]),hBroad[2][j+1], 0, color = 'lightgrey', label = '$Zh_2>Zh_1$')
                        flag2 = False
                    else:
                        axs[i].fill_between((hBroad[1][k],hBroad[1][k+1]),hBroad[2][j+1], 0, color = 'lightgrey')
...
def Zh2DDistribution():
    ...
    for i in range(3): # Loops on the diffrent targets
        ...
        hBroad = axs[i].hist2d(data['Zh'], data['Zh_1'], weights = data['FalPos']/data["Acc"], bins = (binsX, binsY) , cmap=cm) 
        ...
        hBroad2 = axs[i].hist2d(data['Zh'], data['Zh_1'], weights = data['FalPos']/(data["Acc"]*total), bins = (binsX, binsY) , cmap=cm) 
    ...
    fig.align_ylabels(axs[:])
    fig.savefig(outputDirectory + "Zh2Ddistribution-w.pdf", bbox_inches = 'tight')
    fig.savefig(outputDirectory + "Zh2Ddistribution-w.png", bbox_inches = 'tight', dpi = 600)
...
def PtBroad2DZhSumExtra():
    ...
    binsX = [.1, .2, .3, .4, .5, .6, .8, 1.]
    for i in range(3): # Loops on the diffrent targets
        fig, axs = plt.subplots(2, 1, constrained_layout = True, gridspec_kw={'height_ratios': [1, 9]})
        fig.set_size_inches(width, height)
        ...
        hError = plt.hist2d(data['Zh'], data['Zh_1'], bins = (binsX, binsY), weights = data['Error']) 
        hBroad = axs[1].hist2d(data['Zh'], data['Zh_1'], bins = (binsX, binsY), weights = data['Broad'] , cmap=cm) 
        ...
        for j in range(len(hBroad[2])-1):
            for k in range(len(hBroad[1])-1):
                ...
                if j<= k+1 and hBroad[1][k+1]/2. <= hBroad[2][j+1]: 
                    ...
                    if flag1:
                        axs[1].fill_between((hBroad[1][k],hBroad[1][k+1]), 1, hBroad[2][j],color='grey', label = '$Zh_1>Zh_{Sum}$')
                        flag1 = False
                    else:
                        axs[1].fill_between((hBroad[1][k],hBroad[1][k+1]), 1, hBroad[2][j], color = 'grey') 
                else:
                    if flag2:
                        axs[1].fill_between((hBroad[1][k],hBroad[1][k+1]),hBroad[2][j+1], 0, color = 'lightgrey', label = '$Zh_2>Zh_1$')
                        flag2 = False
                    else:
                        axs[1].fill_between((hBroad[1][k],hBroad[1][k+1]),hBroad[2][j+1], 0, color = 'lightgrey')
                # elif k < jbb:
                    # print("hola")
                    # axs[0].fill_between((hBroad[1][k],hBroad[1][k+1]), 0.5, hBroad[2][j],color='grey')
    # Set the labels for the three plots
        axs[1].set_xlabel(r'$Zh_{SUM}$', loc = "center", fontsize = 15)
        axs[1].set_ylabel(r'$Zh_1$', loc = "center", fontsize = 15)
    
        axs[1].legend(frameon = True, loc = 'lower right', fontsize = 10, framealpha = 1)
        ...
        axs[1].tick_params(right = False, top = False, left = False, bottom = False, which = 'minor')
        ...
        cmap_white = cm 
        hBroad1DError = axs[0].hist2d(x, y, bins = (binsX, YBin), weights = errors, cmap=cmap_white) 
        hBroad1D = axs[0].hist2d(x, y, bins = (binsX, YBin), weights = weight, cmap=cmap_white) 
        ...
        axs[0].tick_params(right = False, top = False, left = False, bottom = False, which = 'minor')
        ...
        file.Close()
        fig.savefig(outputDirectory + "PtBroad2DZhSum_" + tarList[i]+ "-wExtra.pdf", bbox_inches = 'tight')
        fig.savefig(outputDirectory + "PtBroad2DZhSum_" + tarList[i]+ "-wExtra.png", bbox_inches = 'tight', dpi = 400)
        fig.savefig(outputDirectory + "PtBroad2DZhSum_" + tarList[i]+ "-wExtra.eps", bbox_inches = 'tight', format = 'eps')
