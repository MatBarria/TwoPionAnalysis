...
void PtBroadeningFullIntegrated(TString inputDirectory, TString plotDirectory) {
    ...
    for(int nPion = 1; nPion <= N_PION; nPion++) {
        for(int i = 0 ; i < N_STARGETS ; i++) {
            ...
            TH1F* histSolid  = (TH1F*) inputFile->Get(Form("meanPt2_%s_%i", SolTargets[i],
                                                            nPion));
            TH1F* histLiquid = (TH1F*) inputFile->Get(Form("meanPt2_%s_%i", LiqTargets[i], 
                                                            nPion));
            //Calculate the Broadening (subtract of the means)
            TH1F* histBroadening = new TH1F(Form("histBroadening_%i", i), "", 1, Zh_MIN, Zh_MAX);
...
void PtBroadeningZh(TString inputDirectory, TString outputDirectory) {
    ...
    TFile* outputFileHist = new TFile(outputDirectory + "Pt_broad_hist_Zh.root", "RECREATE");
    gROOT->cd();
    TFile* outputFile = new TFile(outputDirectory + "Pt_broad_Zh.root", "RECREATE");
    gROOT->cd();
    ...
    for(int nPion = 1; nPion <= N_PION; nPion++) { // Loops in every number of pion
        for(int i = 0 ; i < N_STARGETS ; i++) { // Loops in the diffent targets
            ...
            TH1F* histBroadening = new TH1F(Form("histBroadening_%i",i), "", N_Zh, Zh_BINS);
            histBroadening->Add(histSolid, histLiquid, 1, -1);

            TGraphErrors* graph = (TGraphErrors*) TH1TOTGraph(histBroadening);
            outputFileHist->cd();
            histBroadening->Write(Form("PtBroad_Zh_%s_%i", SolTargets[i] ,nPion));
            outputFile->cd();
            graph->Write(Form("PtBroad_Zh_%s_%i", SolTargets[i], nPion-1)); 
            gROOT->cd();
