...
void PtBroadeningFullIntegrated(TString inputDirectory, TString plotDirectory,
    ...
    gROOT->cd();
    // A^(1\3) C , Fe , Pb
    double nuclearSizes[N_STARGETS] = {TMath::Power(12.01, 1. / 3.),
                                       TMath::Power(55.845, 1. / 3.),
    ...
    for (int nPion = 1; nPion <= N_PION; nPion++) {
        for (int i = 0; i < N_STARGETS; i++) {
            ...
            TH1F *histSolid =
                (TH1F *)inputFile->Get(Form("meanPt2_%s_%i", SolTargets[i], nPion));
            TH1F *histLiquid =
                (TH1F *)inputFile->Get(Form("meanPt2_%s_%i", LiqTargets[i], nPion));
