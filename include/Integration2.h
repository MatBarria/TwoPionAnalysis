        ...
        for (int Q2Counter = 0; Q2Counter < N_Q2; Q2Counter++) {     // Loops in every Q2 bin
            for (int NuCounter = 0; NuCounter < N_Nu; NuCounter++) { // Loops in every Nu bin
                for (int ZhCounter = 0; ZhCounter < N_Zh;
                    ...
                    // For every bin of Q2, Nu, Zh and number of pion, generate a histogram for
                    // Pt2
...
void NuZhIntegration(TFile *inputFile, TFile *outputFile, const char *target) {
    ...
    for (int nPion = 1; nPion <= N_PION; nPion++) {
        ...
        for (int Q2Counter = 0; Q2Counter < N_Q2; Q2Counter++) { // Loops in every Q2 bin
            TH1F *histPt2Integrated = new TH1F(Form("corr_data_Pt2_%s_%i", target, Q2Counter),
                                               "", N_Pt2, Pt2_MIN, Pt2_MAX);
...
void CallPhiIntegration(TString inputDirectory, TString outputDirectory) {
    ...
    TFile *inputFile = new TFile(inputDirectory + "corr_data_Phi_Inter.root", "READ");
