...
void fitAndCutoff(std::string target, TFile *inputFile, TFile *outputFile) {
    ...
    for (int nPion = 1; nPion <= N_PION; nPion++) {              // Loops in number of pion
        for (int ZhCounter = 0; ZhCounter < N_Zh; ZhCounter++) { // Loops in Zh Bins
            ...
            hist = (TH1F *)inputFile->Get(
                Form("corr_data_Pt2_%s_%i_%i", targetArr, ZhCounter, nPion));
            ...
            for (int i = 0; i < N_FITS; i++) {
                ...
                TF1 *FFit =
                    new TF1("FFit", "[0]*TMath::Exp(-x/[1])", MinFitRange, MaxFitRange);
                ...
                if ((FFit->GetChisquare() / FFit->GetNDF() < maxChiQndf)) {
                    ...
                    outputFile->cd();
                    histCutoff->Write(
                        Form("corr_data_Pt2_%s_%i_%i_fit%i", targetArr, ZhCounter, nPion, i));
                    gROOT->cd();
            ...
            if (ValidFits == 0) {
                ...
                histCutoff->Write(
                    Form("corr_data_Pt2_%s_%i_%i_clean", targetArr, ZhCounter, nPion));
                ...
                histCutoff->Write(
                    Form("corr_data_Pt2_%s_%i_%i_interpolated", targetArr, ZhCounter, nPion));
                gROOT->cd();
