...
void GetInterpolatedPt2Distribution(TH1F* hist) {
    ...
    for(int binCounter = 1 ; binCounter <= hist->GetNbinsX() ; binCounter++) { // Loops in every bin
        if(hist->GetBinContent(binCounter) == 0) { // If the bin if empty
            ...
            double interpolatedPt2 = GetInterpolatedPt2Point(missingBinCenter, prevBin,
                    nextBin, prevBinCenter, nextBinCenter);
...
void fitAndCutoff(std::string target, TFile* inputFile, TFile* outputFile) {
    ...
    for(int nPion = 1; nPion <= N_PION; nPion++) { // Loops in number of pion
        for(int ZhCounter = 0 ; ZhCounter < N_Zh ; ZhCounter++) { // Loops in Zh Bins
            ...
            hist = (TH1F*) inputFile->Get(Form("corr_data_Pt2_%s_%i_%i", targetArr, ZhCounter,
                        nPion));
            ...
            for(int i = 0 ; i < N_FITS ; i++) {
                ...
                if((FFit->GetChisquare()/FFit->GetNDF() < maxChiQndf)) {
                    ...
                    outputFile->cd();
                    histCutoff->Write(Form("corr_data_Pt2_%s_%i_%i_fit%i", targetArr, ZhCounter,
                                nPion,i));
                    gROOT->cd();
            ...
            if(ValidFits == 0){
                ...
                histCutoff->Write(Form("corr_data_Pt2_%s_%i_%i_clean", targetArr, ZhCounter,
                            nPion));
                GetInterpolatedPt2Distribution(histCutoff);
                histCutoff->Write(Form("corr_data_Pt2_%s_%i_%i_interpolated", targetArr,  
                            ZhCounter, nPion));
                gROOT->cd();
                ...
                meanPt2CleanInterpolated->SetBinContent(ZhCounter+1, histCutoff->GetMean());
                meanPt2CleanInterpolated->SetBinError(ZhCounter+1, histCutoff->GetMeanError());
            ...
            histInterpolated = (TH1F*) histCutoff->Clone(); // Create another copy of the histogram to interpolated it
                                                            //Interpolating
            ...
            histCutoff->Write(Form("corr_data_Pt2_%s_%i_%i_clean", targetArr, ZhCounter, 
                        nPion));
            histInterpolated->Write(Form("corr_data_Pt2_%s_%i_%i_interpolated", targetArr, 
                        ZhCounter, nPion));
            ...
            meanPt2Clean->SetBinContent(ZhCounter+1, histCutoff->GetMean());
            meanPt2Clean->SetBinError(ZhCounter+1,   histCutoff->GetMeanError());
            meanPt2CleanInterpolated->SetBinContent(ZhCounter+1, histInterpolated->GetMean());
            meanPt2CleanInterpolated->SetBinError(ZhCounter+1, histInterpolated->GetMeanError());
...
void CallPt2Processing(TString inputDirectory, TString outputDirectory) {
    ...
    TFile* outputFile  = new TFile(outputDirectory + "meanPt2_Zh_processed.root", "RECREATE");
    gROOT->cd();
