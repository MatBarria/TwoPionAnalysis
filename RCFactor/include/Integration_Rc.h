...
void PhiIntegration(TFile* inputFile, TFile* outputFile, char target[]) {
  ...
  for(int nPion = 1; nPion <= N_PION; nPion++) { // Loops in every number of pion
    for(int Q2Counter = 0 ; Q2Counter < N_Q2; Q2Counter++) { // Loops in every Q2 bin
	  ...
	  TH1F* histPt2 = new TH1F(Form("corr_data_Pt2_%s_%i%i%i_%i", target, Q2Counter, NuCounter,
		                   ZhCounter, nPion), "", N_Pt2, Pt2_BINS);
          for(int Pt2Counter = 0 ; Pt2Counter < N_Pt2 ; Pt2Counter++) { // Loops in every Pt2 bin
            // Take the his for this bins
	    TH1F* histPhi = (TH1F*) inputFile->Get(Form("DataCorrRc_%s_%i%i%i%i_%i", target, 
...
void Q2NuIntegration(TFile* inputFile, TFile* outputFile, char target[]) {
  ...
  for(int nPion = 1; nPion <= N_PION; nPion++) { // Loops in every number of pion
    ...
    for(int ZhCounter = 0 ; ZhCounter < N_Zh ; ZhCounter++) { // Loops in every Zh bin
      ...
      TH1F* histPt2Integrated = new TH1F("histPt2Sum", "", N_Pt2, Pt2_BINS);
      for(int NuCounter = 0 ; NuCounter < N_Nu ; NuCounter++) { // Loops in every Nu bin
	 for(int Q2Counter = 0 ; Q2Counter < N_Q2 ; Q2Counter++) { // Loops in every Q2 bin
          // Sum the histograms for every bin of Q2 and Nu
	  TH1F* histPt2 = (TH1F*) inputFile->Get(Form("corr_data_Pt2_%s_%i%i%i_%i", target, 
...
void ZhIntegration(TFile* inputFile, TFile* outputFile, char target[]) {
  ...
  for(int nPion = 1; nPion <= N_PION; nPion++) { // Loops in every number of pion
    ...
    TH1F* histPt2Integrated = new TH1F(Form("corr_data_%s_Pt2", target), "", N_Pt2, Pt2_BINS);
    ...
    for(int ZhCounter = ZH_SUM ; ZhCounter < N_Zh ; ZhCounter++) { // Loops in every Zh bin
      ...
      TH1F* histPt2;
      if(UseCutOff == 1) {
	histPt2 = (TH1F*) inputFile->Get(Form("corr_data_Pt2_%s_%i_%i_interpolated", target, 
					      ZhCounter, nPion));
      }
      if(UseCutOff == 2) { 
	histPt2 = (TH1F*) inputFile->Get(Form("corr_data_Pt2_%s_%i_%i_clean", target, ZhCounter, 
				              nPion)); 
      }
      if(UseCutOff == 3) { 
	histPt2 = (TH1F*) inputFile->Get(Form("corr_data_Pt2_%s_%i_%i", target, ZhCounter, nPion)); 
      }
