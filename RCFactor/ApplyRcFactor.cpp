...
// g++ -Wall -fPIC -I./include `root-config --cflags` ApplyRcFactor.cpp -o ./bin/ApplyRcFactor
// `root-config --glibs` ./include/Integration_Rc.h
            ...
            for (int NuCounter = 0; NuCounter < N_Nu; NuCounter++) {     // Loops Nu bins
                for (int ZhCounter = 0; ZhCounter < N_Zh; ZhCounter++) { // Loops Zh bins
                    for (int Pt2Counter = 0; Pt2Counter < N_Pt2;
                        ...
                        // TH1F *histData = (TH1F*)
                        // fileData->Get(Form("DataCorr_%s_%i%i%i%i_%i", targetArr, Q2Counter,
                        // NuCounter, ZhCounter, Pt2Counter, nPion));
                        TH1F *histRc = (TH1F *)fileRcFactors->Get(
                            Form("RcFactor_%s_%i%i%i%i_%i", targetArr, Q2Counter, NuCounter,
                                 ZhCounter, Pt2Counter, nPion));

                        if (histRc == NULL || histData == NULL || EmptyHist(histRc) == 1 ||
                            EmptyHist(histData) == 1) {
                            delete histData;
                            delete histRc;
                            continue;
                        }
