...
from include_rc import inputDirectory, outputDirectory, binning, SaveFigure
...
def PtBroadZhTarSplitAccRatio():
    ...
    for i in range(3): # Loops on the diffrent targets
        ...
        for j in range(nPion): # Loops on the number of pions
            ...
            axs[i].plot(x,yAC/yRC, marker = "o", linestyle = "", markerfacecolor = colorList[j],
                       color = colorList[j], markersize = 5, label = labelList[j])
            # axs[i].hist(x, bins = binning["Zh"], weights = yAC/yRC, histtype="step", 
                       # color = colorList[j], label = labelList[j])
    ...
    SaveFigure(fig, outputDirectory, "RatioAccRC.pdf")
