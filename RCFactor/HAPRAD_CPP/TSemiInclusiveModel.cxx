...
void SemiInclusiveModel(Double_t q2, Double_t X, Double_t Y, Double_t Z, Double_t pt2, Double_t mx2, Double_t pl, Double_t& A, Double_t& Ac,
  ...
  // std::cout<<"Se llamo a SemiInclusiveModel"  << std::endl;
  ...
  // std::cout<<"iniciante: " << initialized << std::endl;
  ...
  if (!initialized) {
    ...
    // if(tuple==NULL) {std::cout << "no reconoce la tupla" << std::endl;}
    ...
    for (int i = 0; i < tuple->GetEntries(); i++) {
      tuple->GetEntry(i);
      // std::cout << rQ2 << "-" << rXb << "-" << rZh <<"-" << rPt << "-" << rA << "-" << rAc << "-" << rAcc <<  std::endl;
      values[0] = rQ2;
      values[1] = rXb;
      values[2] = rZh;
      values[3] = rPt;
