...
void TRadCor::CalculateRCFactor(Double_t E, Double_t x, Double_t Q2, Double_t z, Double_t p_t, Double_t phi, Double_t maxMx2,
  ...
  std::cout << "entro a CalculateRCFactor " << std::endl;
  ...
  // std::cout<< "Nu HAPRAD:" << S_x / 2 / kMassProton << std::endl;
  ...
  if ((Nu2 + S_x * x) * (SQ(z) * Nu2 - SQ(kMassPion) - SQ(p_t)) < 0) {
    std::cout<< "hay una raiz negativa" << std::endl;
  }
// std::cout<< "SQ Z Nu2" <<  (SQ(z) * Nu2 ) << std::endl;
// std::cout<< "Sq masspion" <<  SQ(kMassPion)  << std::endl;
// std::cout<< "Sq pt" <<  SQ(p_t) << std::endl;

  fKinError = false;
  fParametersError = false;
  if(Mx2 <= maxMx2) {
    std::cout << "NO CUMPLEEEEEEE" << std::endl;
  }
  try {
    if (Mx2 > maxMx2) {
      std::cout << "CUMPLEEEEEEE" << std::endl;
      Initialization();
      SPhiH();
    }
