// g++ -Wall -fPIC -I../include `root-config --cflags` CountEmptyBinsCut.cpp -o ../bin/CountEmptyBinsCut  `root-config --glibs` ../include/Binning.h
            ...
            for(int NuCounter = 0; NuCounter < N_Nu; NuCounter++ ) {
                for(int ZhCounter = 0; ZhCounter < N_Zh; ZhCounter++ ) {
                    for(int Pt2Counter = 0; Pt2Counter < N_Pt2; Pt2Counter++ ) {
                        TH1F* noCutHist = (TH1F*) noCutFile->Get(Form("DataCorr2_%s_%i%i%i%i_%i", 
                                    targetArr, Q2Counter, NuCounter, ZhCounter, Pt2Counter,
                                    nPion));
                        TH1F* cutHist = (TH1F*) cutFile->Get(Form("DataCorr2_%s_%i%i%i%i_%i",
                                    targetArr, Q2Counter, NuCounter, ZhCounter, Pt2Counter,
                                    nPion));
                        if(noCutHist == NULL) {
                            delete noCutHist;
