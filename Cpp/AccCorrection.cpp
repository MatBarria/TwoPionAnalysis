...
int AcceptanceCorrection(std::string target, TFile *fileOutput) {
  ...
  TFile *fileSimul = new TFile(
      dataDirectory + Form("SimulTuple_%s.root", simulTarget), "READ");
  gROOT->cd();
  ...
  std::cout << Form("Simul target %s, Target %s", simulTarget, targetArr)
            << std::endl;
    ...
    for (int Q2Counter = 0; Q2Counter < N_Q2;
      ...
      for (int NuCounter = 0; NuCounter < N_Nu;
        ...
        for (int ZhCounter = 0; ZhCounter < N_Zh;
          ...
          std::cout << "Bin selected: " << gen << Q2Counter << NuCounter
                    << ZhCounter << std::endl;

          // Select the cuts for each bin
          Q2Cut =
              Form("Q2>%f&&Q2<%f", Q2_BINS[Q2Counter], Q2_BINS[Q2Counter + 1]);
          NuCut =
              Form("Nu>%f&&Nu<%f", Nu_BINS[NuCounter], Nu_BINS[NuCounter + 1]);
          ZhCut =
              Form("Zh>%f&&Zh<%f", Zh_BINS[ZhCounter], Zh_BINS[ZhCounter + 1]);
          ...
          TNtuple *ntupleSimul_gen =
              (TNtuple *)fileSimul->Get("ntuple_sim_gen");
          TNtuple *ntupleSimul_rec =
              (TNtuple *)fileSimul->Get("ntuple_sim_rec");
          ...
          TEventList *evntSimul_gen =
              (TEventList *)gDirectory->Get("listSimul_gen");
          TEventList *evntSimul_rec =
              (TEventList *)gDirectory->Get("listSimul_rec");
          ...
          for (int Pt2Counter = 0; Pt2Counter < N_Pt2;
            ...
            if (EmptyHist(histData) == 1) {
              ...
              // Generate histograms of the all dectected pion, all generated
              // pion, and the pions that was correct dectected
            ...
            // Calculate a factor that reprensent how many of the detected as
            // N pion events are truly N pions events
            ...
            histDataCorr2->Write(Form("DataCorr2_%s_%i%i%i%i_%i", targetArr,
                                      Q2Counter, NuCounter, ZhCounter,
                                      Pt2Counter, gen));
            histDataCorr->Write(Form("DataCorr_%s_%i%i%i%i_%i", targetArr,
                                     Q2Counter, NuCounter, ZhCounter,
                                     Pt2Counter, gen));
            histFalPos->Write(Form("FalPosFactor_%s_%i%i%i%i_%i", targetArr,
