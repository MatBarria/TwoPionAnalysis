...
// g++ -Wall -fPIC -I../include `root-config --cflags` AccCorrectionME.cpp -o ,./bin/AccCorrectionME  `root-config --glibs` ../include/Acc.h
...
int AcceptanceCorrection(std::string target, TFile* fileOutput) {
  ...
  TFile* fileSimul  = new TFile(dataDirectory + Form("MoreEnergySimul_%s.root", simulTarget), 
						      "READ");
  gROOT->cd();
      ...
      for(int NuCounter = 0; NuCounter < N_Nu; NuCounter++) { // Loops in every Nu bin
        for(int ZhCounter = 0; ZhCounter < N_Zh; ZhCounter++) { // Loops in every Zh bin
          ...
          for(int Pt2Counter = 0; Pt2Counter < N_Pt2; Pt2Counter++) { // Loops in every Pt2 bin
            ...
            // Generate histograms of the all dectected pion, all generated pion,
	    // and the pions that was correct dectected
