...
// g++ -Wall -fpic  `root-config --cflags` duplicateSimulTuple.cpp -o
// ../bin/duplicateSimulTuple `root-config --glibs` For the target name use (D,C,Fe,Pb)
...
int main(int argc, char *argv[]) {
    ...
    TFile *simFile =
        new TFile(Form(dataDirectory + "SimulTuple_%sdeltaZ.root", targetArr), "READ");
    ...
    if (simulTuple == NULL) {
        std::cout << "la ptm, que paso ahora\n";
    }
    ...
    TFile *outputFile =
        new TFile(dataDirectory + Form("SimulTuple_%s.root", targetArr), "RECREATE");
    gROOT->cd();
