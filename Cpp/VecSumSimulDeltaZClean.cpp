...
// for the hadrons variables calculate vectorial momentum and use it as the hadron momentum for
// the event The code run over the simulation generated with the code GetSimpleTuple
// https://github.com/utfsm-eg2-data-analysis/GetSimpleTuple
// It can be compile with
// g++ -Wall -fPIC  `root-config --cflags` VecSumSimulDeltaZ.cpp -o ./bin/VecSumSimulDeltaZ
// `root-config --glibs` For the target name use (D,C,Fe,Pb)
...
int main(int argc, char *argv[]) {
    ...
    for (int folder = 1; folder < 10; folder++) { // Loops in every directory
        for (int sim = 154; sim < 157; sim++) {   // Loops in every simulation of the directory
            ...
            if (targetArr[0] == 'D') {
                if (folder < 4) {
                    inputName = Form(
                        "/eos/user/m/mbarrial/out/GetSimpleTuple_HSim/D2_pb%i/prunedD_%i.root",
                        folder, sim);
                } else {
                    inputName = Form("/eos/user/m/mbarrial/out/GetSimpleTuple_HSim/"
                                     "D2_pb%i_yshiftm03/prunedD_%i.root",
                                     folder, sim);
                }
            } else {
                if (folder < 4) {
                    inputName = Form(
                        "/eos/user/m/mbarrial/out/GetSimpleTuple_HSim/%s%i/pruned%s_%i.root",
                        targetArr, folder, targetArr, sim);
                } else {
                    inputName = Form("/eos/user/m/mbarrial/out/GetSimpleTuple_HSim/"
                                     "%s%i_yshiftm03/pruned%s_%i.root",
                                     targetArr, folder, targetArr, sim);
                }
            }
            // std::cout << "Checking directory " << folder << "  " << sim << std::endl;
            inputName = Form("/home/matias/proyecto/Piones/Data/Simul/pruned%s_%i.root",
                             targetArr, sim);
            ...
            int isPion;
            for (int i = 0; i < simulTuple->GetEntries();
                 i++) { // Loops in every generated particle
                isPion = 0;
                ...
                for (int k = 0; k < (int)vars[0]; k++) {
                    ...
                    TVector2 *tmpVec = new TVector2(
                        tmpPt[k] * TMath::Cos((tmpPhi[k] + 180) * TMath::DegToRad()),
    ...
    // Save the Ntuple
    // TFile *fileOutput= new
    // TFile(Form("/eos/user/m/mbarrial/Data/Acc/SimulTuple_%sdeltaZ.root", targetArr),
    // "RECREATE");
