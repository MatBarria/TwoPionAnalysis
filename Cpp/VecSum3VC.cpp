...
// g++ -Wall -fPIC -I../include `root-config --cflags` VecSum3VC.cpp -o ../bin/VecSum  `root-config --glibs` ../include/Binning.h
...
int main(int argc, char* argv[]) {
    ...
    float tmpEvnt, evnt, Q2Evnt, NuEvnt, XbEvnt, ZhEvnt, Pt2Evnt, PhiEvnt, YCEvnt, VCTMEvnt, 
          VCRDEvnt, VCHHEvnt, DZEvnt;
    float tmpZh[5], tmpPt[5], tmpPhi[5] ;
    float deltaZcut = 3.;
    int dummyval = -999;
    const char* VarList = "Q2:Nu:Zh:Pt2:PhiPQ:YC:VC_TM:Zh_1:Zh_2:Zh_3:Xb:VC_RD:VC_HH";
    ...
    tuple->SetBranchAddress("VC_TM",&VCTMEvnt);
    tuple->SetBranchAddress("VC_RD",&VCRDEvnt);
    tuple->SetBranchAddress("TargType",&VCHHEvnt);
