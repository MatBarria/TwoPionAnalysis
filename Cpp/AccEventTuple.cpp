...
int AccEvnt(std::string target) {
    ...
    Pt2_BINS_Corr[0] = 0.;
    Phi_BINS_Corr[0] = -180;
    for(int i = 1; i <= N_Pt2_Corr; i++) {
        Pt2_BINS_Corr[i] = Pt2_BINS_Corr[i-1] + Delta_Pt2_Corr;
        std::cout << "Pt Bin " << i << ": " << Pt2_BINS_Corr[i] << std::endl;
    }
    for(int i = 1; i <= N_Phi_Corr; i++) {
        Phi_BINS_Corr[i] = Phi_BINS_Corr[i-1] + Delta_Phi_Corr;
    ...
    TFile* fileOutput = new TFile(Form(dataDirectory + "VecSum_%s_Acc.root", targetArr),
                "RECREATE");
    gROOT->cd();
    for(int nPion = 1; nPion <= N_PION ; nPion++) { // Loops in every number of generated pions
        ...
        for(int i = 0; i < ntupleData->GetEntries() ; i++) { // Loops in every detected paricle
            ...
            for(int Pt2Counter = 0; Pt2Counter < N_Pt2_Corr; Pt2Counter++) { // Loops in every Pt2 bin

                if(vars[3] > Pt2_BINS_Corr[Pt2Counter] && vars[3] < Pt2_BINS_Corr[Pt2Counter+ 1]) {
                    Pt2Bin = Pt2Counter;
                    break;
                }
            ...
            for(int PhiCounter = 0; PhiCounter < N_Phi_Corr; PhiCounter++) { // Loops in every Phi bin

                if(vars[4] > Phi_BINS_Corr[PhiCounter] && vars[4] < Phi_BINS_Corr[PhiCounter+ 1]) {
                    PhiBin = PhiCounter;
                    break;
                }
