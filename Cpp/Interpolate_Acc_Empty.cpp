// g++ -Wall -fPIC -I../include `root-config --cflags` Interpolate_Acc_Empty.cpp -o
...
int ApplyInterpolateAcc(std::string target, TFile *fileData, TFile *fileAccInter,
                        TFile *fileDataCorr);
int InterpolationAcc(std::string target, TFile *fileData, TFile *fileAccInter);
int SetEmptyHistogram(TH1F *hist);

int main() {

    TFile *fileData = new TFile(inputDirectory + "corr_data_Phi.root", "READ");
    TFile *fileAccInter = new TFile(sysDirectory + "AccInter/AccInter.root", "RECREATE");
    TFile *fileDataCorr = new TFile(sysDirectory + "AccInter/corr_data_Phi.root", "RECREATE");
    gROOT->cd();
    ...
    for (int i = 0; i < 2 * N_STARGETS; i++) {
        ...
        InterpolationAcc(Targets[i], fileData, fileAccInter);
    }
    fileAccInter->Close();
    delete fileAccInter;
    TFile *fileAccInterRead = new TFile(sysDirectory + "AccInter/AccInter.root", "READ");

    for (int i = 0; i < 2 * N_STARGETS; i++) {
        std::cout << "Applying Acc Interpolated " << Targets[i] << "\n";
        ApplyInterpolateAcc(Targets[i], fileData, fileAccInterRead, fileDataCorr);
    }
...
int InterpolationAcc(std::string target, TFile *fileData, TFile *fileAccInter) {
    ...
    char targetArr[n + 1];
    strcpy(targetArr, target.c_str());
    float Acc, AccPrevQ2, AccNextQ2, AccPrevNu, AccNextNu, AccPrevZh, AccNextZh, AccPrevPt2,
        AccNextPt2, AccPrevPhi, AccNextPhi, AccSum;
    float AccErr, AccErrPrevQ2, AccErrNextQ2, AccErrPrevNu, AccErrNextNu, AccErrPrevZh,
        AccErrNextZh, AccErrPrevPt2, AccErrNextPt2, AccErrPrevPhi, AccErrNextPhi, AccErrSum;
    float AccNei[10];
    float AccErrNei[10];
    int counter;

    float AccArray[N_Q2][N_Nu][N_Zh][N_Pt2][N_Phi];
    float AccErrArray[N_Q2][N_Nu][N_Zh][N_Pt2][N_Phi];
                    ...
                    for (int Pt2Counter = 0; Pt2Counter < N_Pt2; Pt2Counter++) { // Pt2 bins;
                        ...
                        for (int PhiCounter = 1; PhiCounter <= N_Phi; PhiCounter++) {
                            ...
                            if (histData->GetBinContent(PhiCounter) != 0) {
                                ...
                                AccErrArray[Q2Counter][NuCounter][ZhCounter][Pt2Counter]
                                           [PhiCounter - 1] = hist->GetBinError(PhiCounter);
                                continue;
                            } else {
                                AccArray[Q2Counter][NuCounter][ZhCounter][Pt2Counter]
                                        [PhiCounter - 1] = -1;
