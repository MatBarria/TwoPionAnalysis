// g++ -Wall -fPIC -I../include `root-config --cflags` PhiHistCorr.cpp -o
// ../bin/PhiHistCorr `root-config --glibs` ../include/Binning.h
...
int main() {
  ...
  TFile *fileOutput =
      new TFile(outputDirectory + "corr_data_Phi_Evnt.root", "RECREATE");
  gROOT->cd();
...
int PhiHistCorr(std::string target, TFile *fileOutput) {
  ...
  float ZhEvnt, Q2Evnt, NuEvnt, Pt2Evnt, PhiEvnt, YCEvnt, Acc, FalPos, Vc,
      weight;
    ...
    for (int NuCounter = 0; NuCounter < N_Nu;
      ...
      for (int ZhCounter = 0; ZhCounter < N_Zh;
        ...
        for (int Pt2Counter = 0; Pt2Counter < N_Pt2;
          ...
          delete gROOT->FindObject(Form("histPhi_%i%i%i%i", Q2Counter,
                                        NuCounter, ZhCounter, Pt2Counter));
          histPhi[Q2Counter][NuCounter][ZhCounter][Pt2Counter] =
              new TH1F(Form("histPhi_%i%i%i%i", Q2Counter, NuCounter, ZhCounter,
                            Pt2Counter),
                       "", 12, -180, 180);
        } // End Pt2 loop
      }   // End Q2 loop
  ...
  for (int nPion = 1; nPion <= N_PION; nPion++) {
    ...
    TNtuple *ntupleData =
        (TNtuple *)fileData->Get(Form("ntuple_%i_pion", nPion));
    ...
    for (int i = 0; i < ntupleData->GetEntries();
         i++) { // Loops in every detected paricle

      ntupleData->GetEntry(i);

      if (Vc != VCCut || ZhEvnt > 1 || Q2Evnt < Q2_BINS[0] ||
          Q2Evnt > Q2_BINS[N_Q2] || NuEvnt < Nu_BINS[0] ||
          NuEvnt > Nu_BINS[N_Nu] || TMath::Abs(YCEvnt) > 1.4) {
        continue;
      }
      ...
      for (int Pt2Counter = 0; Pt2Counter < N_Pt2;
           Pt2Counter++) { // Loops in every Pt2 bin
        if (Pt2Evnt > Pt2_BINS[Pt2Counter] &&
            Pt2Evnt < Pt2_BINS[Pt2Counter + 1]) {
          Pt2Bin = Pt2Counter;
          break;
        }
      } // End Pt2 loop
      ...
      for (int NuCounter = 0; NuCounter < N_Nu;
        ...
        for (int ZhCounter = 0; ZhCounter < N_Zh;
          ...
          for (int Pt2Counter = 0; Pt2Counter < N_Pt2;
               Pt2Counter++) { // Loops in every Pt2 bin
            fileOutput->cd();
            histPhi[Q2Counter][NuCounter][ZhCounter][Pt2Counter]->Write(
                Form("DataCorr2_%s_%i%i%i%i_%i", targetArr, Q2Counter,
